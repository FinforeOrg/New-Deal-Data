...
if(isset($_POST['action'])&&($_POST['action']=="add")){
	...
	$success = $g_trans->add_partner($_POST,"bank",$validation_passed,$g_view['err']);
	if(!$success){
		die("Cannot add bank to the deal");
	}
	if($validation_passed){
		$g_view['msg'] = "Bank added";
	}else{
...
if(isset($_POST['action'])&&($_POST['action']=="delete")){
	...
	$success = $g_trans->remove_partner($_POST,"bank",$g_view['msg']);
	if(!$success){
...
if(isset($_POST['action'])&&($_POST['action']=="flip_sellside_status")){
	...
	$success = $g_trans->partner_sellside_flag($_POST,"bank",$g_view['msg']);
	if(!$success){
...
if(isset($_POST['action'])&&($_POST['action']=="flip_is_insignificant_status")){
	...
	$success = $g_trans->partner_is_insignificant_flag($_POST,"bank",$g_view['msg']);
	if(!$success){
...
if(isset($_POST['action'])&&($_POST['action']=="role")){
	...
	$success = $deal_support->set_deal_partner_role($_POST,"bank",$g_view['msg']);
	if(!$success){
...
/***
sng:20/may/2010
No need to fetch list of banks as we implement ajax hint
*******/
///////////////////////////////////////////////////////////
/***********************
sng:16/sep/2011
...
$success = $deal_support->front_get_deal_partner_roles("bank",$g_view['deal_type']['deal_cat_name'],$g_view['roles'],$g_view['roles_count']);
if(!$success){
	die("Cannot get the partner roles");
}
include("admin/deal_bank_popup_view.php");
?>