...
$g_view['err'] = array();
///////////////////////////////////////
if(isset($_POST['action'])&&($_POST['action']=="create_ghost")){
	
	$success = $g_mem->create_ghost_member_from_registration($_POST,$g_view['msg']);
	if(!$success){
		die("Could not create ghost");
	}else{
		/***
		sng:28/july/2010
		since we are deleting the data, we cannot show the page again, so we show the list
		***/
		header("Location: favoured_unactivated_registration_list.php");
		exit;
	}
...
if(isset($_POST['action'])&&($_POST['action']=="delete")){
	...
	if(!$success){
		die("Cannot delete registration request data");
	}
	else
	{
	  header("location:favoured_unactivated_registration_list.php");
...
$g_view['content_view'] = "admin/favoured_new_registration_detail_view.php";
