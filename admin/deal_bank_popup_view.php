...
function lookup(inputString) {
	if(inputString.length == 0) {
		...
		$('#bank_searching').html("searching...");
		$.post("ajax/get_company_for_deal.php", {name: ""+inputString+"",type: "bank"}, function(data){
			$('#bank_searching').html("");
			if(data.length >0) {
				$('#suggestions').show();
...
<td colspan="5">
<!--add bank form-->
<form method="post" action="deal_bank_popup.php?transaction_id=<?php echo $_REQUEST['transaction_id'];?>" enctype="multipart/form-data">
...
<input type="text" name="firm_name" id="firm_name" class="txtbox" style="width:200px;" value="<?php echo $g_view['input']['firm_name'];?>" onkeyup="lookup(this.value);" onblur="hide_suggestion();" /><br />
		<span id="bank_searching"></span><br />
...
<td>
<form method="post" action="deal_bank_popup.php">
