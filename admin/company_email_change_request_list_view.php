...
if($g_view['data_count']==0){
	...
	for($i=0;$i<$total;$i++){
		...
		<input type="hidden" name="myaction" value="reject" />
		...
		<td>
		<form action="" method="post">
		<input type="hidden" name="myaction" value="accept" />
		<input type="hidden" name="mem_id" value="<?php echo $g_view['data'][$i]['mem_id'];?>" />
		...
		<td>
		<form action="" method="post">
		<input type="hidden" name="myaction" value="accept_and_update" />
		<input type="hidden" name="mem_id" value="<?php echo $g_view['data'][$i]['mem_id'];?>" />
		<input type="submit" name="submit" value="Accept and Update" />
		</form>
		</td>
		</tr>
		</table>
	...
	if($g_view['start'] > 0){
		...
		?>
		<a href="company_email_change_request_list.php?start=<?php echo $prev_offset;?>">Prev</a>
		<?php
	}
	if($g_view['data_count'] > $g_view['num_to_show']){
		$next_offset = $g_view['start'] + $g_view['num_to_show'];
		?>
		&nbsp;&nbsp;&nbsp;<a href="company_email_change_request_list.php?start=<?php echo $next_offset;?>">Next</a>
		<?php
	}
	?>
	</td>
