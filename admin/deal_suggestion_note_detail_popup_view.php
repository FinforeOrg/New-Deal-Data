...
if(0 == $g_view['data_count']){
	?>
	<tr><td>None suggested</td></tr>
	<?php
}else{
	$current_label = "";
	for($i=0;$i<$g_view['data_count'];$i++){
		if($g_view['data'][$i]['label']!=$current_label){
			?>
			<tr><td colspan="2"><span style="font-weight:bold;"><?php echo $g_view['data'][$i]['label'];?></span></td></tr>
			<?php
			$current_label = $g_view['data'][$i]['label'];
		}
		?>
		<tr>
		<td>
		<textarea style="width:500px; height:100px;"><?php echo $g_view['data'][$i]['data'];?></textarea>
		</td>
		<td>
		On <?php echo date("Y-m-d",strtotime($g_view['data'][$i]['date_suggested']));?><br />
