...
if($g_view['data_count']==0){
	...
	for($i=0;$i<$total;$i++){
		...
		<td><?php echo $g_view['data'][$i]['short_name'];?></td>
		<td>
	...
	if($g_view['start'] > 0){
		...
		?>
		<a href="blf_suggestion_list.php?start=<?php echo $prev_offset;?>">Prev</a>
		<?php
	}
	if($g_view['data_count'] > $g_view['num_to_show']){
		$next_offset = $g_view['start'] + $g_view['num_to_show'];
		?>
		&nbsp;&nbsp;&nbsp;<a href="blf_suggestion_list.php?start=<?php echo $next_offset;?>">Next</a>
		<?php
	}
	?>
	</td>
