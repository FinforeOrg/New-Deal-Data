...
if($g_view['data_count']==0){
	...
	for($i=0;$i<$total;$i++){
		...
		<td>
		<a href="" onclick="return deal_duplicate_lawfirm_popup('<?php echo $g_view['data'][$i]['id'];?>');">Manage Law Firms</a>
		</td>
	...
	if($g_view['start'] > 0){
		...
		?>
		<a href="misc_list_deal_duplicate_law_firms.php?start=<?php echo $prev_offset;?>">Prev</a>
		<?php
	}
	if($g_view['data_count'] > $g_view['num_to_show']){
		$next_offset = $g_view['start'] + $g_view['num_to_show'];
		?>
		&nbsp;&nbsp;&nbsp;<a href="misc_list_deal_duplicate_law_firms.php?start=<?php echo $next_offset;?>">Next</a>
		<?php
	}
	?>
	</td>
