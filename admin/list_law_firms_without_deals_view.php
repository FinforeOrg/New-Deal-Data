...
if($g_view['data_count']==0){
	...
	for($i=0;$i<$total;$i++){
		...
		<td>
		<form method="post" action="" onsubmit="return confirm_deletion_msg('Are you sure you want to delete the law firm <?php echo $g_view['data'][$i]['name'];?>?\nThis cannot be undone');">
	...
	if($g_view['start'] > 0){
		...
		?>
		<a href="list_law_firms_without_deals.php?start=<?php echo $prev_offset;?>">Prev</a>
		<?php
	}
	if($g_view['data_count'] > $g_view['num_to_show']){
		$next_offset = $g_view['start'] + $g_view['num_to_show'];
		?>
		&nbsp;&nbsp;&nbsp;<a href="list_law_firms_without_deals.php?start=<?php echo $next_offset;?>">Next</a>
		<?php
	}
	?>
	</td>
