...
class deal_support{
	...
	*******************/
	public function admin_list_all_participant_notification_detail_paged($start_from,$num_to_show,&$data_arr,&$data_count){
		$db = new db();
		$q = "select a.*,c.name as firm_name from ".TP."transaction_add_firms_alert as a left join ".TP."company as c on (a.partner_id=c.company_id) order by notify_id";
		$success = $db->select_query_limited($q,$start_from,$num_to_show);
		if(!$success){
			return false;
		}
		$data_count = $db->row_count();
	...
	public function admin_add_participant_notification_detail($data_arr,&$validation_passed,&$err_arr){
		$db = new db();
		
		$validation_passed = true;
		if($data_arr['firm_name']==""){
			$validation_passed = false;
			$err_arr['firm_name'] = "Please specify the firm";
		}else{
			if($data_arr['partner_id']==""){
				//the firm name was specified but the id was not found
				$validation_passed = false;
				$err_arr['firm_name'] = "The firm does not exist in the database";
			}
		}
		...
		$q = "insert into ".TP."transaction_add_firms_alert set
		...
		region_name='".$data_arr['region']."',
		country_name='".$data_arr['country']."',
		deal_sector='".$data_arr['sector']."',
		deal_industry='".$data_arr['industry']."'";
		
		$success = $db->mod_query($q);
		return $success;
	}
	
	public function admin_delete_participant_notification_detail($notify_id){
		$db = new db();
		$q = "delete from ".TP."transaction_add_firms_alert where notify_id='".$notify_id."'";
		$success = $db->mod_query($q);
		return $success;
	}
	
	public function notify_participants($firm_id,$deal_id){
		$db = new db();
		/***************************
		first check if the deal requires notification or not
		******************************/
		$q = "select email_participating_syndicates from ".TP."transaction where id='".$deal_id."'";
		$success = $db->select_query($q);
		if(!$success){
			return false;
		}
		if(!$db->has_row()){
			//no such transaction?, well, we just get out
			return false;
		}
		$row = $db->get_row();
		if($row['email_participating_syndicates']!='y'){
			//no need for notification
			return true;
		}
		/*********************
		The table tombstone_transaction_add_firms_alert holds condition to search for deals. So
		we do not write query to see which records match the deal parameters. Rather, we first get the records
		for the given firm and then take each one by one and see if the data in the records matches the deal parameters.
		
		We have tweaked front_deal_search_paged to accept the deal id
		**************************/
		$q = "select * from ".TP."transaction_add_firms_alert where partner_id='".$firm_id."'";
		...
		$from = $admin_data['mem_related_email'];
		global $g_http_path;
		
		for($j=0;$j<$row_count;$j++){
			$data = array();
			$data['deal_id'] = $deal_id;
			$data['deal_cat_name'] = $data_arr[$j]['deal_cat_name'];
			$data['deal_subcat1_name'] = $data_arr[$j]['deal_subcat1_name'];
			$data['deal_subcat2_name'] = $data_arr[$j]['deal_subcat2_name'];
			$data['region'] = $data_arr[$j]['region_name'];
			$data['country'] = $data_arr[$j]['country_name'];
			$data['sector'] = $data_arr[$j]['deal_sector'];
			$data['industry'] = $data_arr[$j]['deal_industry'];
			//the key names in $data matches the keynames expected by front_deal_search_paged
			
			$temp_data = array();
			$temp_count = 0;
			
			$success = $trans->front_deal_search_paged($data,0,1,$temp_data,$temp_count,0);
			if($success&&$temp_count==1){
				//send email
				$headers = "";
				$headers .= "From: ".$admin_email."\r\n";
				$subject = "deal-data.com company / firm added to deal";
				$message = "Hello\r\n\r\n";
				$message.="This is notify you that your firm has been added as a advisor to a deal. Please follow the link to see the detail.\r\n\r\n";
				$message.=$g_http_path."/deal_detail.php?deal_id=".$deal_id."\r\n\r\n";
				$message.="Many thanks,\r\n\r\n";
				$message.="The Admin team at Deal-data.com";
				$to = $data_arr[$j]['email'];
