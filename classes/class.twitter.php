...
function get_tweets($twitter_id, 
                    ...
                    $tweetImageHtmlStart="<img src=\"",
                    $tweetImageHtmlEnd="\" align='left' style='margin-right:10px;' />",
                    $tweetMiddleHtml="</span><span class=\"tweet-details\">",
       ...
       if ( $twitter_xml = twitter_status($twitter_id) ) {
        ...
        foreach ($twitter_xml->status as $key => $status) {
            if ($includeReplies == true | substr_count($status->text,"@") == 0 | strpos($status->text,"@") != 0) {
                ...
                $result.=$tweetStartHtml.$tweetImageHtmlStart.$status->user->profile_image_url.$tweetImageHtmlEnd.$message.$tweetMiddleHtml/*.date($dateFormat,strtotime($status->created_at))*/.$tweetEndHtml;
                ++$i;
